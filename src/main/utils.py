...
def jpg_image_to_array(image_path,sz):
  ...
  #print(im_arr.shape)
  im_arr = im_arr.reshape((3,img.size[1],img.size[0]))
  assert im_arr.shape == (3,sz,sz)
...
def get_dataset(root_dir,split,valid_set=False):
    labels = pd.read_csv(os.path.join(root_dir,split+'Labels.csv'),delimiter=',')
   
    print('checking if .ny file exists')

    x_name = os.path.join(root_dir, subfd_threecls, "{}_{}_x.npy".format(RESCALE, split))
    y_name = os.path.join(root_dir, subfd_threecls, "{}_{}_y.npy".format(RESCALE, split))
    print(x_name)
...
def load_dataset(dataset, class_number, mean, std, hyper_para):
    if(dataset=='abnormal'):# change path files according to path on your device
...
def choose_network(model_type, pre_trained_flag):
    ...
    if(model_type=='alexnet'):
        ...
        model = resnet26(num_classes=2)
        path = '/home/students/student3_15/00_astar/00_baseline/finetune/model_notrain.pkl'
        model.load_state_dict(torch.load(path))
        # model = torchvision.models.resnet50(pretrained=pre_trained_flag)
        # new_classifier = nn.Sequential(*list(model.children())[:-3])
        # model.classifier = new_classifier
...
def choose_classifier(dataset, class_number, model_type, model, classifier, hyper_para, train_data, test_data, test_label, no_train_data, no_test_data, inm, relu, m, s):
